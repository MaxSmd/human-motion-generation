...
The packed format and all loading / subset / mirror / crop / pad logic now live
in `shared.data.humanml3d` (model-agnostic, reused by mardm). rmg's only
specialization is defaulting the representation to T+R (the paper's main
result); other models supply their own. `HumanML3DSample`, `CollatedBatch` and
`collate` are re-exported from `shared.data` so existing imports are unchanged.
"""
