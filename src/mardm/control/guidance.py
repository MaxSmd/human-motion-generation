...
def generate_guided(
    ...
    *,
    ...
    for step, timestep in enumerate(torch.linspace(0, 1, timesteps, device=device)):
        ...
        # Transformer pass stays out of the graph — we optimize z directly.
        with torch.no_grad():
        ...
        # Fixed noise per AR step so the inner optimization is deterministic.
        noise = torch.randn(int(flat_mask.sum()), d, device=device)

        if g.inner_iters > 0 and control.num_constraints > 0:
            z_opt = z.detach().clone().requires_grad_(True)
            optimizer = torch.optim.Adam([z_opt], lr=g.lr)
            context = latents.detach().reshape(b * l, d)
            for it in range(g.inner_iters):
                ...
                loss.backward()
                optimizer.step()
                if g.verbose and (it == 0 or it == g.inner_iters - 1):
                    print(f"[guidance] step {step + 1}/{timesteps} it {it + 1}/{g.inner_iters} "
                          f"L_s={float(loss):.4f}", flush=True)
            z = z_opt.detach()
    ...
    if g.post_iters > 0 and control.num_constraints > 0:
        ...
        for it in range(g.post_iters):
            ...
            if g.verbose and (it == 0 or it == g.post_iters - 1):
                print(f"[guidance] post it {it + 1}/{g.post_iters} L_s={float(loss):.4f}", flush=True)
